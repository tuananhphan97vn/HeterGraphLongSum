...
def run_training(model, train_loader, valid_loader, valset, hps, train_dir):
	...
	for epoch in range(1, hps.n_epochs + 1):
		...
		for i, (G_ws , G_wp , G_sp, index) in enumerate(train_loader):

			iter_start_time = time.time()
			model.train()

			if hps.cuda:
				G_ws.to(torch.device("cuda"))
				G_wp.to(torch.device("cuda"))
				G_sp.to(torch.device("cuda"))

			outputs = model.forward(G_ws , G_wp , G_sp)  # [n_snodes, 2]
			snode_id = G_ws.filter_nodes(lambda nodes: nodes.data["dtype"] == 1) 

			label = G_ws.ndata["label"][snode_id].sum(-1)  # [n_nodes]
			# print('label shape ', label)
			G_ws.nodes[snode_id].data["loss"] = criterion(outputs, label).unsqueeze(-1)  # [n_nodes, 1]
			loss = dgl.sum_nodes(G_ws, "loss")  # [batch_size, 1]
			loss = loss.mean()
...
def run_eval(model, loader, valset, hps, best_loss, best_F, non_descent_cnt, saveNo):
	...
	with torch.no_grad():
		...
		for i, (G_ws , G_wp , G_sp, index) in enumerate(loader):
			if hps.cuda:
				G_ws.to(torch.device("cuda"))
				G_wp.to(torch.device("cuda"))
				G_sp.to(torch.device("cuda"))
			tester.evaluation( G_ws , G_wp , G_sp, index, valset)
...
def main():
	...
	# Where to find data
	parser.add_argument('--data_dir', type=str, default='../data-summarization/data/cnndm',help='The dataset directory.')
	parser.add_argument('--cache_dir', type=str, default='../data-summarization/cache/CNNDM',help='The processed dataset directory')
	parser.add_argument('--embedding_path', type=str, default='../data-summarization/embedding/embed_1/glove.6B.100d.txt', help='Path expression to external word embedding.')
	...
	parser.add_argument('--word_emb_dim', type=int, default=100, help='Word embedding size [default: 300]')
	...
	parser.add_argument('--doc_max_timesteps', type=int, default=100,help='max length of documents (max timesteps of documents)')
	...
	# File paths
	if args.full_data == 'sub':
		DATA_FILE = os.path.join(args.data_dir, "train.sub.label.jsonl")
		VALID_FILE = os.path.join(args.data_dir, "val.sub.label.jsonl")
	elif args.full_data =='full':
	...
	if args.full_data == 'sub':
		train_w2s_path = os.path.join(args.cache_dir, "train.sub.w2s.tfidf.jsonl")
		val_w2s_path = os.path.join(args.cache_dir, "val.sub.w2s.tfidf.jsonl")
	if args.full_data == 'full':
	...
	if hps.model == "HSG":
		...
		dataset = ExampleSet(DATA_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, train_w2s_path ,hps.passage_length)
		...
		del dataset
		valid_dataset = ExampleSet(VALID_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, val_w2s_path, hps.passage_length)
...
if __name__ == '__main__':
	main()
	# data = pickle.load(open('test.sav','rb'))
	# print(data)
	# print('type data', type(data))
	# # for i in data[0].nodes():
	# #     print(i, data[0].edges.)
	# print('edges ', data[0].edges)
