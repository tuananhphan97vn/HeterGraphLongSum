...
def run_test(model, dataset, loader, model_name, hps):
	...
	with torch.no_grad():
		...
		for i, (G_ws , G_wp , G_sp, index) in enumerate(loader):
			...
			s = time.time()
			tester.evaluation(G_ws , G_wp , G_sp, index, dataset, blocking=hps.blocking)
			e = time.time()
			print(i, e- s)
...
def main():
	...
	if hps.model == "HSG":
		...
		dataset = ExampleSet(DATA_FILE, vocab, hps.doc_max_timesteps, hps.sent_max_len, FILTER_WORD, test_w2s_path , hps.passage_length)
		loader = torch.utils.data.DataLoader(dataset, batch_size=hps.batch_size, shuffle=False, num_workers=16,collate_fn=graph_collate_fn)
