...
class SLTester(TestPipLine):
    ...
    def evaluation(self, G_ws , G_wp , G_sp, index, dataset, blocking=False):
        """
            :param G: the model
        ...
        """
        self.batch_number += 1
        outputs = self.model.forward(G_ws , G_wp , G_sp)
        # logger.debug(outputs)
        snode_id = G_ws.filter_nodes(lambda nodes: nodes.data["dtype"] == 1)
        label = G_ws.ndata["label"][snode_id].sum(-1)            # [n_nodes]
        G_ws.nodes[snode_id].data["loss"] = self.criterion(outputs, label).unsqueeze(-1)    # [n_nodes, 1]
        loss = dgl.sum_nodes(G_ws, "loss")    # [batch_size, 1]
        loss = loss.mean()
        self.running_loss += float(loss.data)

        G_ws.nodes[snode_id].data["p"] = outputs  #(num_sent node , 2)
        glist = dgl.unbatch(G_ws)
        for j in range(len(glist)):
            ...
            N = len(snode_id)
            p_sent = g.ndata["p"][snode_id]  #output for single doc in batch 
